...
pw.MultiPage buildGridClassicTemplate(
        ...
        children: [
          ...
          if (signatureImage != null) ...[
            ...
            buildSignatureWidget(signatureImage, signaturePosition,
                imageHeight: 40 * fontScale, labelFontSize: labelFont),
          ],
  ...
  return pw.MultiPage(
    ...
    footer: (context)
    {
      final rawTotalPages = (context.pagesCount / copies).round();
      final totalPagesPerCopy = rawTotalPages < 1 ? 1 : rawTotalPages;
      // only one page we don't need total page counters
      if(totalPagesPerCopy == 1)
      {
        return pw.SizedBox();
      }
      final pageInCopy = ((context.pageNumber - 1) % totalPagesPerCopy) + 1;
      return pw.Container(
        alignment: pw.Alignment.centerRight,
        margin: const pw.EdgeInsets.only(top:5,bottom: 0,left: 0,right: 0),
        child: pw.Text(
          showFooterBranding
              ? "Page $pageInCopy of $totalPagesPerCopy  -  Generated by Invoiso"
              : "Page $pageInCopy of $totalPagesPerCopy",
