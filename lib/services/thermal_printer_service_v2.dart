...
class _NetworkPrintRowState extends State<_NetworkPrintRow> {
  ...
  Future<void> _send() async {
    ...
    try {
      final settings =
      await ThermalPrinterServiceV2._fetchReceiptSettings(widget.invoice);
      ...
      if (!useImage) {
        await ThermalPrinterServiceV2._printToNetworkText(
        ...
        if (!mounted) return;
        await ThermalPrinterServiceV2._printToNetwork(
          context: context,
          ip: _ipController.text.trim(),
          port: int.tryParse(_portController.text.trim()) ?? 9100,
          invoice: widget.invoice,
