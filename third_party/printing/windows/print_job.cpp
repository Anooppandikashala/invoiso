...
// Requesting an arbitrary custom paper size (dmPaperSize = 0) is only
// honored by drivers that have that exact size pre-registered. When the
// requested size matches a well-known ISO/ANSI page, ask for it by its
// standard DMPAPER_* id instead so the driver applies its own calibrated
// margins/tray mapping for that paper (fixes clipped edges when printing
// e.g. A5 invoices on a printer loaded with plain A5/A4 sheets).
static WORD MapStandardPaperId(double portraitWidthPt,
  ...
  static const StdSize kSizes[] = {
      {DMPAPER_A4, 595.28, 841.89},
      {DMPAPER_A5, 419.53, 595.28},
      {DMPAPER_A6, 297.64, 419.53},
      {DMPAPER_LETTER, 612.0, 792.0},
  };
  const double kTolerancePt = 2.0;  // ~0.7mm
  for (const auto& s : kSizes) {
    if (std::abs(portraitWidthPt - s.w) <= kTolerancePt &&
        std::abs(portraitHeightPt - s.h) <= kTolerancePt) {
      return s.id;
    }
  }
  return 0;
...
bool PrintJob::printPdf(const std::string& name,
  ...
  if (usePrinterSettings) {
    ...
    auto stdPaperId = MapStandardPaperId(portraitWidth, portraitHeight);
    if (stdPaperId != 0) {
      dm->dmFields = DM_ORIENTATION | DM_PAPERSIZE;
      dm->dmPaperSize = stdPaperId;
    } else {
      dm->dmFields =
          DM_ORIENTATION | DM_PAPERSIZE | DM_PAPERLENGTH | DM_PAPERWIDTH;
      dm->dmPaperSize = 0;
      dm->dmPaperWidth = static_cast<short>(round(portraitWidth * 254 / pdfDpi));
      dm->dmPaperLength = static_cast<short>(round(portraitHeight * 254 / pdfDpi));
    }
  }
